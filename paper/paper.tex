...
\textbf{Exact and quasi-exact methods.} \citet{lee2023forwardprop} prove that SNNs have well-defined gradients w.r.t.\ weights via the IFT and propose a forward-propagation algorithm that computes exact gradients by exploiting causality between spike times. \citet{wunderlich2021eventprop} derive EventProp, an event-based backpropagation algorithm that computes exact gradients using the adjoint method with proper partial derivative jumps through spike discontinuities, achieving competitive results on Yin-Yang and MNIST. \citet{bauer2023exodus} develop EXODUS, which computes gradients equivalent to BPTT by applying IFT within SLAYER's framework, eliminating the need for ad-hoc gradient scaling. Our work shares the IFT foundation but differs in the vectorized GPU implementation that exploits closed-form inter-event analysis for acceleration, and the escape-noise mechanism for existence gradients which we show is important for training convergence.
...
\bibitem[{Lee et~al.(2023)}]{lee2023forwardprop}
J.~H. Lee, S.~Haghighatshoar, and A.~Karbasi.
\newblock Exact gradient computation for spiking neural networks via
  forward propagation.
\newblock In {\em Proceedings of the 26th International Conference on
  Artificial Intelligence and Statistics (AISTATS)}, PMLR 206:1812--1831, 2023.

\bibitem[{Wunderlich and {Pehle}(2021)}]{wunderlich2021eventprop}
T.~Wunderlich and J.~Pehle.
\newblock Event-based backpropagation can compute exact gradients for spiking
  neural networks.
\newblock {\em Scientific Reports}, 11:11075, 2021.

\bibitem[{Bauer et~al.(2023)}]{bauer2023exodus}
F.~C. Bauer, G.~Lenz, S.~Haghighatshoar, and S.~Sheik.
\newblock {EXODUS}: Stable and efficient training of spiking neural networks.
\newblock {\em Frontiers in Neuroscience}, 17:1110444, 2023.
